...
`;
    ...
    // Scrub API key from any logged error to avoid credential exposure in server logs
    let safeErrorMessage = error.message;
    if (safeErrorMessage && process.env.GEMINI_API_KEY) {
      safeErrorMessage = safeErrorMessage.replace(new RegExp(process.env.GEMINI_API_KEY, 'g'), '***REDACTED***');
    }
    
    let safeErrorData = error.response?.data;
    if (safeErrorData && typeof safeErrorData === 'object') {
      safeErrorData = JSON.parse(JSON.stringify(safeErrorData).replace(new RegExp(process.env.GEMINI_API_KEY, 'g'), '***REDACTED***'));
    }

    const status = error.response?.status || 500;
    if (status === 401) {
      console.error('Chatbot API Error: 401 Unauthenticated. Please check if your GEMINI_API_KEY is valid and not restricted.');
    } else if (status === 403) {
      console.error('Chatbot API Error: 403 Forbidden. Permissions issue or billing not active.');
    } else if (status === 429) {
      console.error('Chatbot API Error: 429 Too Many Requests. Quota or rate limit exceeded.');
    } else if (status >= 500) {
      console.error('Chatbot API Error: 5xx Provider Failure. Google server error.');
    } else if (error.code === 'ECONNABORTED' || error.message.includes('timeout')) {
      console.error('Chatbot API Error: Connection Timeout. Google server took too long to respond.');
    } else {
      console.error('Chatbot API Error:', safeErrorMessage, safeErrorData);
    }
